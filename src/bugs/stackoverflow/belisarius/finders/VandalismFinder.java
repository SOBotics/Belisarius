...
public class VandalismFinder {
	...
	public VandalisedPost findReasons() {
    	...
    	{
    		for (String reason : getReasons(post.getPostType(), post.getTitle(), post.getLastTitle(), true, false, 2.0)) {
    			vandalisedPost.addReason(reason);
    		}
    	} 
    	
    	if (post.getBody() != null && post.getLastBody() != null) {
    		for (String reason : getReasons(post.getPostType(), post.getBody(), post.getLastBody(), false, true, 1.0)) {
    			vandalisedPost.addReason(reason);
    		}
    	}
	...
	private List<String> getReasons(String postType, String target, String original, boolean isCheckOnTitle, boolean isCheckOnBody, double quantifier) {
		List<String> reasons = new ArrayList<String>();
		
		if (target != "" && original != "") {
			ReasonList reasonList = new SOBoticsReasonList(postType, target, original, isCheckOnTitle, isCheckOnBody, quantifier);
